...
test('starts a real download over HTTP, sees real progress, and DELETE cancels it', { timeout: 60000 }, async () => {
  ...
  try {
    const startRes = await fetch(`${base}/downloads`, {
      ...
      body: JSON.stringify({ torrentUrl: 'https://webtorrent.io/torrents/sintel.torrent', outputDir }),
    });
    ...
    assert.equal(afterWait.piecesCompleted, piecesAtCancel, 'no further pieces should complete after cancellation');
  } finally {
