...
test('downloads the complete archive.org torrent using only web-seeding, no peers', { timeout: 60000 }, async () => {
  ...
  assert.ok(torrent.urlList.length > 0, 'fixture must have BEP19 url-list entries to run this test');
  ...
  try {
    ...
    const downloadedVideo = await readFile(join(outputDir, '1953_Movie_Trailers_Starting_Monday_00_35_40_28_3mb.mp4'));
