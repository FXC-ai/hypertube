...
test('downloads the complete Sintel torrent from the real swarm end to end', { timeout: 240000 }, async () => {
  const torrentPath = fileURLToPath(new URL('../fixtures/sintel.webtorrent.io.torrent', import.meta.url));
  const torrent = parseTorrentFile(readFileSync(torrentPath));
  const infoHash = Buffer.from(torrent.infoHash, 'hex');
  const peerId = generatePeerId();

  const announceResult = await announceUdpTracker('udp://tracker.opentrackr.org:1337', {
    infoHash,
    peerId,
    port: 6881,
  ...
  try {
    ...
    const totalWrittenSize = torrent.files.reduce((sum, file) => sum + statSync(join(outputDir, file.path)).size, 0);
    ...
    assert.ok(Array.isArray(probed.streams) && probed.streams.length > 0, 'ffprobe should be able to open Sintel.mp4 and list its streams');
    assert.ok(probed.streams.some((s) => s.codec_type === 'video'), 'expected at least one decodable video stream');
  } finally {
