...
export async function announce(trackerUrls, params, options = {}) {
  ...
  for (const trackerUrl of dedupe(trackerUrls)) {
    ...
    if (!announcer) {
      errors.push(new TrackerError('Unsupported tracker scheme (only http(s):// and udp:// are implemented)', { trackerUrl }));
      continue;
    }
    try {
  ...
  throw new TrackerError(`All ${errors.length} tracker(s) failed: ${errors.map((err) => err.message).join('; ')}`);
}
