...
export function parseTorrentFile(buffer) {
  ...
  const announceList = top.has('announce-list')
    ? parseAnnounceList(top.get('announce-list'))
    : undefined;
    ...
    const path = pathParts
      .map((part, partIndex) => {
        if (!Buffer.isBuffer(part)) {
          throw new TorrentFileError(
            `Malformed .torrent file: files[${index}].path[${partIndex}] is not a byte string`,
          );
        }
...
function parseAnnounceList(raw) {
  ...
  return raw.map((tier, tierIndex) => {
    if (!Array.isArray(tier)) {
      throw new TorrentFileError(
        `Malformed .torrent file: announce-list[${tierIndex}] is not a list`,
      );
    }
    ...
    return tier.map((url, urlIndex) => {
      if (!Buffer.isBuffer(url)) {
        throw new TorrentFileError(
          `Malformed .torrent file: announce-list[${tierIndex}][${urlIndex}] is not a byte string`,
        );
      }
...
function parseUrlList(raw) {
  ...
  if (!Array.isArray(raw)) {
    throw new TorrentFileError(
      'Malformed .torrent file: "url-list" is not a byte string or a list',
    );
  }
  ...
  return raw.map((url, index) => {
    if (!Buffer.isBuffer(url)) {
      throw new TorrentFileError(
        `Malformed .torrent file: url-list[${index}] is not a byte string`,
      );
    }
